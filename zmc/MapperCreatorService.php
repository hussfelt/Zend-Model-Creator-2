...
";
	...
	*/
	...
	private function _generatePersist($className) {
		...
		$this->_data.="\t*\n";
		$this->_data.="\t* @param object ".ZendModelCreator2::getNamespace()."\Entity\\" . ucfirst(strtolower($className)) . "\n";
		$this->_data.="\t* @return object ".ZendModelCreator2::getNamespace()."\Entity\\" . ucfirst(strtolower($className)) . "\n";
		$this->_data.="\t**/\n";
		$this->_data.="\tpublic function persist(" . ucfirst(strtolower($className)) . " \$" . strtolower($className) . ")\n";
		$this->_data.="\t{\n";
		$this->_data.="\t\tif (\$" . strtolower($className) . "->get" . ZendModelCreator2::toCamelCase($this->_primary_key) . "() > 0) {\n";
		$this->_data.="\t\t\t\$where = new Where;\n";
		$this->_data.="\t\t\t\$where->equalTo('" . $this->_primary_key . "', \$" . strtolower($className) . "->get" . ZendModelCreator2::toCamelCase($this->_primary_key) . "());\n";
		...
		$this->_data.="\t\t\t\$" . strtolower($className) . "->set" . ZendModelCreator2::toCamelCase($this->_primary_key) . "(\$result->getGeneratedValue());\n";
		$this->_data.="\t\t}\n";
